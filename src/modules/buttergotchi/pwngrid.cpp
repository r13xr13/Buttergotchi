...
esp_err_t pwngridAdvertise(uint8_t channel, String face) {
    ...
    uint8_t *pwngrid_beacon_frame = (uint8_t *)heap_caps_malloc(frame_len, MALLOC_CAP_8BIT | MALLOC_CAP_SPIRAM);
    if (!pwngrid_beacon_frame) { pwngrid_beacon_frame = (uint8_t *)malloc(frame_len); }
    ...
    esp_err_t result;
    for (int i = 0; i < 3; i++) {
        // Use WIFI_IF_STA — AP interface raw TX is unreliable on ESP32-S3
        result = esp_wifi_80211_tx(WIFI_IF_STA, pwngrid_beacon_frame, frame_len, false);
        if (result != ESP_OK) {
...
void initPwngrid() {
    wifiPromiscuousActive = true; // Event handlers registered by ensureWifiPlatform() must be suppressed
    ensureWifiPlatform();
    wifi_init_config_t WIFI_INIT_CONFIG = WIFI_INIT_CONFIG_DEFAULT();
    esp_wifi_init(&WIFI_INIT_CONFIG);
    esp_wifi_set_storage(WIFI_STORAGE_RAM);
    // APSTA required on ESP32-S3 — esp_wifi_80211_tx(WIFI_IF_AP) can crash
    // when promiscuous mode is active due to hardware sequence-number handling.
