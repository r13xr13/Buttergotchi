...
void send_buttergotchi_beacon(uint8_t channel, const char *face, const char *name) {
    JsonDocument json;
    json["pal"] = true;
    json["name"] = name;
    ...
    while (spamRunning) {
        ...
        if (dos_pwnd) {
            ...
            for (int ch = 0; ch < num_channels; ++ch) {
                ...
                send_buttergotchi_beacon(channels[ch], pwnd_faces[0], pwnd_names[0]);
            ...
            for (int i = 0; i < num_faces; ++i) {
                for (int ch = 0; ch < num_channels; ++ch) {
                    if (stop_beacon) { break; }
                    send_buttergotchi_beacon(channels[ch], faces[i], names[i % num_names]);
                    vTaskDelay(200 / portTICK_PERIOD_MS); // Wait 200 ms
                }
            }
        }
...
void displaySpamStatus() {
    ...
    drawTopCanvas();
    drawBottomCanvas();
    tft.fillRect(0, 20, tftWidth, tftHeight - 40, buttergotchiConfig.bgColor);
    tft.setTextSize(1.5);
    tft.setCursor(0, 20);
    tft.setTextColor(buttergotchiConfig.priColor, buttergotchiConfig.bgColor);
    tft.println("Buttergotchi Spam Running...");
...
void send_buttergotchi_beacon_main() {
    // Initialiser NVS
