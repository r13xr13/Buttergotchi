...
void FileMenu::drawIcon(float scale) {
    ...
    // Files
    tft.drawRect(iconX + iconDelta, iconY - iconDelta, iconW, iconH, buttergotchiConfig.priColor);

    tft.fillRect(iconX, iconY, iconW, iconH, buttergotchiConfig.bgColor);
    tft.drawRect(iconX, iconY, iconW, iconH, buttergotchiConfig.priColor);

    tft.fillRect(iconX - iconDelta, iconY + iconDelta, iconW, iconH, buttergotchiConfig.bgColor);
    tft.drawRect(iconX - iconDelta, iconY + iconDelta, iconW, iconH, buttergotchiConfig.priColor);

    // Erase corners
    tft.fillRect(
        iconX + iconDelta + iconW - foldSize, iconY - iconDelta - 1, foldSize, 2, buttergotchiConfig.bgColor
    );
    tft.fillRect(iconX + iconDelta + iconW - 1, iconY - iconDelta, 2, foldSize, buttergotchiConfig.bgColor);

    tft.fillRect(iconX + iconW - foldSize, iconY - 1, foldSize, 2, buttergotchiConfig.bgColor);
    tft.fillRect(iconX + iconW - 1, iconY, 2, foldSize, buttergotchiConfig.bgColor);
