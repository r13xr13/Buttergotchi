...
void updateClockTimezone() {
    ...
    timeClient.setTimeOffset(buttergotchiConfig.tmz * 3600);

    localTime = timeClient.getEpochTime() + (buttergotchiConfig.dst ? 3600 : 0);
...
void updateTimeStr(struct tm timeInfo) {
    if (buttergotchiConfig.clock24hr) {
        // Use 24 hour format
        snprintf(
...
void showDeviceInfo() {
    ...
    area.addLine("HeavyButter Version: " + String(BUTTERGOTCHI_VERSION));
    ...
    area.addLine("Brightness: " + String(buttergotchiConfig.bright) + "%");
    area.addLine("");
#endif

    area.addLine("[GPIO]");
    area.addLine("GROVE_SDA: " + String(buttergotchiConfigPins.i2c_bus.sda));
    area.addLine("GROVE_SCL: " + String(buttergotchiConfigPins.i2c_bus.scl));
    area.addLine("SERIAL TX: " + String(buttergotchiConfigPins.uart_bus.tx));
    area.addLine("SERIAL RX: " + String(buttergotchiConfigPins.uart_bus.rx));
