...
// -------------------------------------------------------------------------
JsonDocument ButtergotchiConfig::toJson() const {
    ...
    setting["usbCompositeMode"] = usbCompositeMode;
    ...
    JsonArray wl = setting["deauthWhitelist"].to<JsonArray>();
    for (const auto &entry : deauthWhitelist) wl.add(entry);

    return jsonDoc;
}

void ButtergotchiConfig::fromFile(bool checkFS) {
    FS *fs;
    if (checkFS) {
        if (!getFsStorage(fs)) {
    ...
    if (!setting["usbCompositeMode"].isNull()) {
        usbCompositeMode = setting["usbCompositeMode"].as<int>();
    } else {
        count++;
        log_e("Fail");
    }
    ...
    if (!setting["deauthWhitelist"].isNull()) {
        ...
        JsonArray wl = setting["deauthWhitelist"].as<JsonArray>();
        for (JsonVariant v : wl) deauthWhitelist.push_back(v.as<String>());
    }
...
void ButtergotchiConfig::saveFile() {
    FS *fs = &LittleFS;
...
void ButtergotchiConfig::factoryReset() {
    FS *fs = &LittleFS;
...
void ButtergotchiConfig::validateConfig() {
    validateDimmerValue();
    validateBrightValue();
    validateTmzValue();
...
void ButtergotchiConfig::setUiColor(uint16_t primary, uint16_t *secondary, uint16_t *background) {
    ButtergotchiTheme::_setUiColor(primary, secondary, background);
    saveFile();
}

void ButtergotchiConfig::setDimmer(int value) {
    dimmerSet = value;
    validateDimmerValue();
    saveFile();
}

void ButtergotchiConfig::validateDimmerValue() {
    if (dimmerSet < 0) dimmerSet = 10;
    if (dimmerSet > 60) dimmerSet = 0;
}

void ButtergotchiConfig::setBright(uint8_t value) {
    bright = value;
    validateBrightValue();
    saveFile();
}

void ButtergotchiConfig::validateBrightValue() {
    if (bright > 100) bright = 100;
}

void ButtergotchiConfig::setAutomaticTimeUpdateViaNTP(bool value) {
    automaticTimeUpdateViaNTP = value;
    saveFile();
}

void ButtergotchiConfig::setTmz(float value) {
    tmz = value;
    validateTmzValue();
    saveFile();
}

void ButtergotchiConfig::validateTmzValue() {
    if (tmz < -12 || tmz > 14) tmz = 0;
}

void ButtergotchiConfig::setDST(bool value) {
    dst = value;
    saveFile();
}

void ButtergotchiConfig::setClock24Hr(bool value) {
    clock24hr = value;
    saveFile();
}

void ButtergotchiConfig::setSoundEnabled(int value) {
    soundEnabled = value;
    validateSoundEnabledValue();
    saveFile();
}

void ButtergotchiConfig::setSoundVolume(int value) {
    soundVolume = value;
    validateSoundVolumeValue();
    saveFile();
}

void ButtergotchiConfig::validateSoundEnabledValue() {
    if (soundEnabled > 1) soundEnabled = 1;
}

void ButtergotchiConfig::validateSoundVolumeValue() {
    if (soundVolume > 100) soundVolume = 100;
}

void ButtergotchiConfig::setWifiAtStartup(int value) {
    wifiAtStartup = value;
    validateWifiAtStartupValue();
    saveFile();
}

void ButtergotchiConfig::validateWifiAtStartupValue() {
    if (wifiAtStartup > 1) wifiAtStartup = 1;
}

#ifdef HAS_RGB_LED
void ButtergotchiConfig::setLedBright(int value) {
    ledBright = value;
    validateLedBrightValue();
    saveFile();
}

void ButtergotchiConfig::validateLedBrightValue() { ledBright = max(0, min(100, ledBright)); }

void ButtergotchiConfig::setLedColor(uint32_t value) {
    ledColor = value;
    validateLedColorValue();
    saveFile();
}

void ButtergotchiConfig::validateLedColorValue() {
    ledColor = max<uint32_t>(0, min<uint32_t>(0xFFFFFFFF, ledColor));
}

void ButtergotchiConfig::setLedBlinkEnabled(int value) {
    ledBlinkEnabled = value;
    validateLedBlinkEnabledValue();
    saveFile();
}

void ButtergotchiConfig::validateLedBlinkEnabledValue() {
    if (ledBlinkEnabled > 1) ledBlinkEnabled = 1;
}

void ButtergotchiConfig::setLedEffect(int value) {
    ledEffect = value;
    validateLedEffectValue();
    saveFile();
}

void ButtergotchiConfig::validateLedEffectValue() {
    if (ledEffect < 0 || ledEffect > 9) ledEffect = 0;
}

void ButtergotchiConfig::setLedEffectSpeed(int value) {
    ledEffectSpeed = value;
    validateLedEffectSpeedValue();
    saveFile();
}

void ButtergotchiConfig::validateLedEffectSpeedValue() {
...
void ButtergotchiConfig::setLedEffectDirection(int value) {
    ledEffectDirection = value;
    validateLedEffectDirectionValue();
    saveFile();
}

void ButtergotchiConfig::validateLedEffectDirectionValue() {
    if (ledEffectDirection > 1 || ledEffectDirection == 0) ledEffectDirection = 1;
    if (ledEffectDirection < -1) ledEffectDirection = -1;
}
#endif

void ButtergotchiConfig::setWebUICreds(const String &usr, const String &pwd) {
    webUI.user = usr;
    webUI.pwd = pwd;
    saveFile();
}

void ButtergotchiConfig::setWifiApCreds(const String &ssid, const String &pwd) {
    wifiAp.ssid = ssid;
    wifiAp.pwd = pwd;
    saveFile();
}

void ButtergotchiConfig::setTerminalLog(bool value) {
    TerminalLog = value;
    saveFile();
}

void ButtergotchiConfig::addWifiCredential(const String &ssid, const String &pwd) {
    wifi[ssid] = pwd;
    saveFile();
}

String ButtergotchiConfig::getWifiPassword(const String &ssid) const {
    auto it = wifi.find(ssid);
    if (it != wifi.end()) return it->second;
    return "";
}

void ButtergotchiConfig::addEvilWifiName(String value) {
    evilWifiNames.insert(value);
    saveFile();
}

void ButtergotchiConfig::removeEvilWifiName(String value) {
    evilWifiNames.erase(value);
    saveFile();
}

void ButtergotchiConfig::setEvilEndpointCreds(String value) {
    evilPortalEndpoints.getCredsEndpoint = value;
    validateEvilEndpointCreds();
    saveFile();
}

void ButtergotchiConfig::validateEvilEndpointCreds() {
...
void ButtergotchiConfig::setEvilEndpointSsid(String value) {
    evilPortalEndpoints.setSsidEndpoint = value;
    validateEvilEndpointCreds();
    saveFile();
}

void ButtergotchiConfig::validateEvilEndpointSsid() {
    if (evilPortalEndpoints.getCredsEndpoint == evilPortalEndpoints.setSsidEndpoint) {
        // on collision reset to defaults
...
void ButtergotchiConfig::setEvilAllowEndpointDisplay(bool value) {
    evilPortalEndpoints.showEndpoints = value;
    saveFile();
}

void ButtergotchiConfig::setEvilAllowGetCreds(bool value) {
    evilPortalEndpoints.allowGetCreds = value;
    saveFile();
}

void ButtergotchiConfig::setEvilAllowSetSsid(bool value) {
    evilPortalEndpoints.allowSetSsid = value;
    saveFile();
}

void ButtergotchiConfig::setEvilPasswordMode(EvilPortalPasswordMode value) {
    evilPortalPasswordMode = value;
    saveFile();
}

void ButtergotchiConfig::validateEvilPasswordMode() {
    if (evilPortalPasswordMode < 0 || evilPortalPasswordMode > 2) evilPortalPasswordMode = FULL_PASSWORD;
}

void ButtergotchiConfig::setEvilGatewayIp(String value) {
    evilPortalGatewayIp = value;
    validateEvilGatewayIp();
    saveFile();
}

void ButtergotchiConfig::validateEvilGatewayIp() {
    IPAddress gatewayIp;
    if (!gatewayIp.fromString(evilPortalGatewayIp)) evilPortalGatewayIp = "172.0.0.1";
}

void ButtergotchiConfig::setStartupApp(String value) {
    startupApp = value;
    saveFile();
}

void ButtergotchiConfig::setStartupAppJSInterpreterFile(String value) {
    startupAppJSInterpreterFile = value;
    saveFile();
}

void ButtergotchiConfig::setWigleBasicToken(String value) {
    wigleBasicToken = value;
    saveFile();
}

void ButtergotchiConfig::setWdgwarsApiKey(String value) {
    wdgwarsApiKey = value;
    saveFile();
}

void ButtergotchiConfig::setDevMode(int value) {
    devMode = value;
    validateDevModeValue();
    saveFile();
}

void ButtergotchiConfig::validateDevModeValue() {
    if (devMode > 1) devMode = 1;
}

void ButtergotchiConfig::setColorInverted(int value) {
    colorInverted = value;
    validateColorInverted();
    saveFile();
}

void ButtergotchiConfig::validateColorInverted() {
    if (colorInverted > 1) colorInverted = 1;
}

void ButtergotchiConfig::setBadUSBBLEKeyboardLayout(int value) {
    badUSBBLEKeyboardLayout = value;
    validateBadUSBBLEKeyboardLayout();
    saveFile();
}

void ButtergotchiConfig::validateBadUSBBLEKeyboardLayout() {
    if (badUSBBLEKeyboardLayout < 0 || badUSBBLEKeyboardLayout > 13) badUSBBLEKeyboardLayout = 0;
}

void ButtergotchiConfig::setBadUSBBLEKeyDelay(uint16_t value) {
    badUSBBLEKeyDelay = value;
    validateBadUSBBLEKeyDelay();
    saveFile();
}

void ButtergotchiConfig::validateBadUSBBLEKeyDelay() {
    if (badUSBBLEKeyDelay < 0) badUSBBLEKeyDelay = 0;
    if (badUSBBLEKeyDelay > 500) badUSBBLEKeyDelay = 500;
}

void ButtergotchiConfig::setBadUSBBLEShowOutput(bool value) {
    badUSBBLEShowOutput = value;
    saveFile();
}
void ButtergotchiConfig::addMifareKey(String value) { MifareKeysManager::addKey(mifareKeys, value); }

void ButtergotchiConfig::validateMifareKeysItems() { MifareKeysManager::validateKeys(mifareKeys); }

void ButtergotchiConfig::addDisabledMenu(String value) {
    // TODO: check if duplicate
    disabledMenus.push_back(value);
    saveFile();
}

void ButtergotchiConfig::addQrCodeEntry(const String &menuName, const String &content) {
    qrCodes.push_back({menuName, content});
    saveFile();
}

void ButtergotchiConfig::removeQrCodeEntry(const String &menuName) {
    size_t writeIndex = 0;
...
void ButtergotchiConfig::addWebUISession(const String &token) {
...
void ButtergotchiConfig::removeWebUISession(const String &token) {
...
bool ButtergotchiConfig::isValidWebUISession(const String &token) {
...
void ButtergotchiConfig::addDeauthWhitelist(const String &entry) {
    if (entry.length() == 0) return;
    String normalized = entry;
    normalized.toLowerCase();
    for (const auto &e : deauthWhitelist) {
        if (e == normalized) return;
    }
    deauthWhitelist.push_back(normalized);
    saveFile();
}

void ButtergotchiConfig::removeDeauthWhitelist(const String &entry) {
    String normalized = entry;
    normalized.toLowerCase();
    for (auto it = deauthWhitelist.begin(); it != deauthWhitelist.end(); ++it) {
        if (*it == normalized) {
            deauthWhitelist.erase(it);
            saveFile();
            return;
        }
    }
...
bool ButtergotchiConfig::isDeauthWhitelisted(const String &entry) const {
    String normalized = entry;
    normalized.toLowerCase();
    for (const auto &e : deauthWhitelist) {
...
std::vector<String> ButtergotchiConfig::getDeauthWhitelist() const {
    return deauthWhitelist;
}
