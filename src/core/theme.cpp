...
void ButtergotchiTheme::removeTheme(void) {
    themeInfo t;
    theme = t;
}
FS *ButtergotchiTheme::themeFS(void) {
    if (theme.fs == 1) return &LittleFS;
    else if (theme.fs == 2) return &SD;
    return &LittleFS; // always get back to safety
}
bool ButtergotchiTheme::openThemeFile(FS *fs, String filepath, bool overwriteConfigSettings) {
    ...
    if (overwriteConfigSettings) {
        uint16_t _priColor = buttergotchiConfig.priColor;
        uint16_t _secColor = buttergotchiConfig.secColor;
        uint16_t _bgColor = buttergotchiConfig.bgColor;
...
#ifdef HAS_RGB_LED
        if (!_th["ledBright"].isNull()) { buttergotchiConfig.ledBright = _th["ledBright"].as<int>(); }
        if (!_th["ledColor"].isNull()) { buttergotchiConfig.ledColor = strtoul(_th["ledColor"], nullptr, 16); }
        if (!_th["ledEffect"].isNull()) { buttergotchiConfig.ledEffect = _th["ledEffect"].as<int>(); }
        if (!_th["ledEffectSpeed"].isNull()) { buttergotchiConfig.ledEffectSpeed = _th["ledEffectSpeed"].as<int>(); }
        if (!_th["ledEffectDirection"].isNull()) {
            buttergotchiConfig.ledEffectDirection = _th["ledEffectDirection"].as<int>();
        }
        ledSetup();
#endif
    }
...
bool ButtergotchiTheme::validateImgFile(FS *fs, String filepath) {
...
void ButtergotchiTheme::_setUiColor(uint16_t primary, uint16_t *secondary, uint16_t *background) {
    priColor = primary;
