...
void applyConfiguredMAC() {
    if (buttergotchiConfig.wifiMAC.length() == 17 && validateMACFormat(buttergotchiConfig.wifiMAC)) {
        uint8_t newMAC[6];
        sscanf(
            buttergotchiConfig.wifiMAC.c_str(),
        ...
        if (esp_wifi_set_mac(WIFI_IF_STA, newMAC) == ESP_OK) {
            Serial.println("[WiFi] Custom MAC applied: " + buttergotchiConfig.wifiMAC);
        } else {
...
bool setCustomMAC(const String &mac) {
    ...
    buttergotchiConfig.wifiMAC = mac;
...
void wifiMACMenu() {
    ...
    if (buttergotchiConfig.wifiMAC != "" && validateMACFormat(buttergotchiConfig.wifiMAC)) {
        currentMAC = buttergotchiConfig.wifiMAC + " (Custom)";
    } else {
