...
// #include "modules/ble/ble_common.h" // stopBLEStack for radio coexistence (module removed)
...
IPAddress AP_GATEWAY(192, 168, 4, 1); // Gateway (standard ESP32 AP range)

AsyncWebServer *server = nullptr; // initialise webserver
const char *host = "buttergotchi";
String uploadFolder = "";
...
**  Cleanly stop WebUI, disconnect WiFi, and tear down BLE before a
**  WiFi feature takes exclusive control of the radio.
**********************************************************************/
void cleanlyStopWebUiForWiFiFeature() {
    // Stop the WebUI if active
    if (server) {
        stopWebUi();
        vTaskDelay(pdMS_TO_TICKS(100));
    }

    // Tear down any active WiFi so the feature can set its own mode
    wifiDisconnect();
    vTaskDelay(pdMS_TO_TICKS(100));

    // BLE stack was removed; no BLE teardown needed
    vTaskDelay(pdMS_TO_TICKS(100));
}
/**********************************************************************
...
bool checkUserWebAuth(AsyncWebServerRequest *request, bool onFailureReturnLoginPage = false) {
    if (request->hasHeader("Cookie")) {
        ...
        int idx = c.indexOf("BUTTERGOTCHISESSION=");
        if (idx != -1) {
            int start = idx + 13;
            int end = c.indexOf(';', start);
            if (end == -1) end = c.length();
            String token = c.substring(start, end);
            if (buttergotchiConfig.isValidWebUISession(token)) { return true; }
        }
    }
...
void drawWebUiScreen(bool mode_ap) {
    ...
    tft.setCursor(padX, currentY);
    tft.print("Url: http://buttergotchi.local");
    currentY += LH * FP + 6;
    ...
    tft.setCursor(padX, currentY);
    tft.print("Usr: " + String(buttergotchiConfig.webUI.user));
    currentY += LH * FP + 6;

    tft.setCursor(padX, currentY);
    tft.print("Pwd: " + String(buttergotchiConfig.webUI.pwd));
...
// Cached probe: scan SD/LittleFS for custom web UI at most once per boot
static bool s_customWebUiProbed = false;
static bool s_customWebUiOnSD = false;

static void probeCustomWebUi() {
    if (s_customWebUiProbed) return;
    s_customWebUiProbed = true;
    if (setupSdCard() && SD.exists("/ButtergotchiWebUI/")) {
        s_customWebUiOnSD = true;
        Serial.println("[WebUI] Custom files found on SD");
    } else if (LittleFS.exists("/ButtergotchiWebUI/")) {
        Serial.println("[WebUI] Custom files found on LittleFS");
    }
}
...
void serveWebUIFile(
    ...
    probeCustomWebUi();
    if (s_customWebUiOnSD) {
        if (SD.exists("/ButtergotchiWebUI/" + filename)) fs = &SD;
    } else if (LittleFS.exists("/ButtergotchiWebUI/" + filename)) {
        fs = &LittleFS;
    }

    if (fs) {
        response = request->beginResponse(*fs, "/ButtergotchiWebUI/" + filename, contentType);
    } else {
        if (filename == "theme.css") {
            String css = ":root{--color:" + color565ToWebHex(buttergotchiConfig.priColor) +
                         ";--sec-color:" + color565ToWebHex(buttergotchiConfig.secColor) +
                         ";--background:" + color565ToWebHex(buttergotchiConfig.bgColor) + ";}";
        ...
        if (gzip) {
            if (!response->addHeader("Content-Encoding", "gzip")) {
                log_e("Failed to add gzip header for %s", filename.c_str());
            }
        }
    }
...
void configureWebServer() {
    ...
    // Diagnostic ping — returns plaintext to verify basic HTTP works
    ...
    server->on("/login", HTTP_POST, [](AsyncWebServerRequest *request) {
        if (request->hasParam("username", true) && request->hasParam("password", true)) {
            ...
            if (username == buttergotchiConfig.webUI.user && password == buttergotchiConfig.webUI.pwd) {
        // Warn if using known default credentials
        if (password == "buttergotchi" || password == "buttergotchinet" || password == "heavybutter") {
            Serial.println("WARNING: Using default credentials! Change immediately.");
            // flash a warning on the web interface
            buttergotchiConfig.warningFlags |= (1 << 0); // flag defaultsWarning
        }

                String token = generateToken();
                AsyncWebServerResponse *response = request->beginResponse(302);
                response->addHeader("Location", "/");
                response->addHeader("Set-Cookie", "BUTTERGOTCHISESSION=" + token + "; Path=/; HttpOnly");
                request->send(response);
                buttergotchiConfig.addWebUISession(token);
                return;
            }
        }
        AsyncWebServerResponse *response = request->beginResponse(302);
    ...
    server->on("/logout", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (request->hasHeader("Cookie")) {
            ...
            if (idx != -1) {
                ...
                if (end == -1) end = c.length();
                String token = c.substring(start, end);
                buttergotchiConfig.removeWebUISession(token);
            }
        }
        AsyncWebServerResponse *response = request->beginResponse(302);
        response->addHeader("Location", "/?loggedout");
        response->addHeader("Set-Cookie", "BUTTERGOTCHISESSION=0; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT");
        request->send(response);
    });
    ...
    // Handshake / WPA Key upload
    server->on("/upload-handshake", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (checkUserWebAuth(request)) {
            String html = "<!DOCTYPE html><html><head><meta name='viewport' content='width=device-width,initial-scale=1'>";
            html += "<title>Buttergotchi - Upload Handshake</title>";
            html += "<style>body{font-family:Arial;margin:20px;background:#1a1a2e;color:#eee}";
            html += "h1{color:#e94560}";
            html += "form{border:2px dashed #e94560;padding:30px;text-align:center;border-radius:10px}";
            html += "input[type=file]{margin:20px 0}";
            html += "input[type=submit]{background:#e94560;color:#fff;border:none;padding:12px 30px;border-radius:5px;font-size:16px;cursor:pointer}";
            html += "a{color:#e94560}";
            html += "</style></head><body>";
            html += "<h1>Upload Handshake / WPA Key</h1>";
            html += "<p>Upload .pcap, .pmkid, .hccapx, or .txt files</p>";
            html += "<form method='POST' action='/upload-handshake' enctype='multipart/form-data'>";
            html += "<input type='file' name='data' required><br><br>";
            html += "<input type='submit' value='Upload'>";
            html += "</form>";
            html += "<br><a href='/'>Back to Files</a>";
            html += "</body></html>";
            request->send(200, "text/html", html);
        }
    });
    server->on("/upload-handshake", HTTP_POST,
        [](AsyncWebServerRequest *request) {
            String msg;
            bool success = request->_tempFile && request->_tempFile.size() > 0;
            if (!setupSdCard()) {
                msg = "No SD card present";
            } else if (success) {
                msg = "File uploaded successfully!";
            } else {
                msg = "Upload failed or file was empty.";
            }
            String html = "<!DOCTYPE html><html><head><meta name='viewport' content='width=device-width,initial-scale=1'>";
            html += "<style>body{font-family:Arial;margin:20px;background:#1a1a2e;color:#eee;text-align:center}";
            html += "h1{color:" + String(success ? "#4ecca3" : "#e94560") + "}";
            html += "a{color:#e94560}</style></head><body>";
            html += "<h1>" + msg + "</h1>";
            html += "<br><a href='/upload-handshake'>Upload another</a> | <a href='/'>Home</a>";
            html += "</body></html>";
            request->send(200, "text/html", html);
        },
        [](AsyncWebServerRequest *request, const String &filename, size_t index, uint8_t *data, size_t len, bool final) {
            if (checkUserWebAuth(request)) {
                if (!index) {
                    if (!setupSdCard()) {
                        Serial.println("Upload failed: no SD card");
                        return;
                    }
                    String dir = "/ButtergotchiPCAP/uploaded";
                ...
                if (request->_tempFile && len > 0) request->_tempFile.write(data, len);
                if (final && request->_tempFile) {
                    Serial.printf("Handshake upload complete: %s (%d bytes)\n", filename.c_str(), request->_tempFile.size());
    ...
    server->on("/wifi", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (checkUserWebAuth(request)) {
            if (request->hasArg("usr") && request->hasArg("pwd")) {
                ...
                buttergotchiConfig.setWebUICreds(usr, pwd);
                request->send(
    ...
    server->begin();
    Serial.printf("Webserver started (port %d)\n", default_webserverporthttp);
}
...
void startWebUi(bool mode_ap) {
    ...
    // Always destroy and recreate the server to ensure clean state
    if (server) {
        stopWebUi();
        vTaskDelay(pdMS_TO_TICKS(100));
    }
    ...
    vTaskDelay(pdMS_TO_TICKS(200));  // let AP interface stabilize
