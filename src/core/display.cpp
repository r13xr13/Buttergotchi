...
void displayTextLine(String txt, bool waitKeyPress) {
    // todo: add newlines to txt if too long
    displayRedStripe(txt, getComplementaryColor2(buttergotchiConfig.priColor), buttergotchiConfig.priColor);
#ifndef HAS_SCREEN
...
int loopOptions(
    ...
    int menuBoxY = tftHeight / 2 - menuSize * (FM * 8 + 4) / 2 - 5;
    if (menuBoxY < 30) menuBoxY = 30;
    if (index > 0)
        tft.fillRoundRect(
            tftWidth * 0.10,
            menuBoxY,
    ...
    while (1) {
        ...
        if (menuType == MENU_TYPE_MAIN) {
            checkReboot();
            if (devModeCounter >= 5 && !buttergotchiConfig.devMode) {
                buttergotchiConfig.setDevMode(true);
            ...
            while (PrevPress && menuType != MENU_TYPE_MAIN) {
                if (millis() - _tmp > 200)
                    tft.drawArc(
                        ...
                        getColorVariation(buttergotchiConfig.priColor),
                        buttergotchiConfig.bgColor
                    );
                vTaskDelay(10 / portTICK_RATE_MS);
            }
            tft.drawArc(
                tftWidth / 2, tftHeight / 2, 25, 15, 0, 360, buttergotchiConfig.bgColor, buttergotchiConfig.bgColor
            );
            LongPress = false;
#endif
        ...
        if (check(NextPress) || check(DownPress)) {
            ...
            if (nextEnabled >= 0) {
                if (!buttergotchiConfig.devMode && nextEnabled <= index) devModeCounter++;
                index = nextEnabled;
            }
            redraw = true;
        }
...
void progressHandler(int progress, size_t total, String message) {
    ...
    if (barWidth < 3) {
        tft.fillRect(6, 27, tftWidth - 12, tftHeight - 33, buttergotchiConfig.bgColor);
        tft.drawRect(18, tftHeight - 47, tftWidth - 36, 17, buttergotchiConfig.priColor);
        displayRedStripe(message, TFT_WHITE, buttergotchiConfig.priColor);
    }
    tft.fillRect(20, tftHeight - 45, barWidth, 13, buttergotchiConfig.priColor);
}
...
Opt_Coord drawOptions(
    ...
    // Keep menu below the top HUD (status bar area ~28px, add margin)
    if (optionsTopY < 35) optionsTopY = 35;
    ...
    tft.setTextColor(fgcolor, bgcolor);
    tft.setTextSize(FM);
    int32_t cursorY = tftHeight / 2 - menuSize * (FM * 8 + 4) / 2;
    if (cursorY < 35) cursorY = 35;
    tft.setCursor(tftWidth * 0.10 + 5, cursorY);
...
void drawSubmenu(int index, std::vector<Option> &options, const char *title) {
    ...
    tft.setTextSize(FP);
    tft.drawPixel(0, 0, 0);
    tft.fillRect(6, 30, tftWidth - 12, 8 * FP, buttergotchiConfig.bgColor);
    ...
    tft.setTextColor(options[firstIndex].enabled ? buttergotchiConfig.secColor : TFT_DARKGREY);
    tft.fillRect(6, middle_up, tftWidth - 12, 8 * FM, buttergotchiConfig.bgColor);
    ...
    tft.setTextColor(options[index].enabled ? buttergotchiConfig.priColor : TFT_DARKGREY);
    tft.fillRect(6, middle - FG * LH / 2 - 1, tftWidth - 12, FG * LH + 5, buttergotchiConfig.bgColor);
    ...
    tft.setTextSize(FM);
    tft.setTextColor(options[thirdIndex].enabled ? buttergotchiConfig.secColor : TFT_DARKGREY);
    tft.fillRect(6, middle_down, tftWidth - 12, 8 * FM, buttergotchiConfig.bgColor);
    tft.drawCentreString(thirdOption, tftWidth / 2, middle_down, SMOOTH_FONT);

    tft.fillRect(tftWidth - 5, 0, 5, tftHeight, buttergotchiConfig.bgColor);
    tft.fillRect(tftWidth - 5, index * tftHeight / menuSize, 5, tftHeight / menuSize, buttergotchiConfig.priColor);

#if defined(HAS_TOUCH)
    tft.drawCentreString("\\/", tftWidth / 2, middle_down + (FM * LH + 6), 1);
    tft.setTextColor(getColorVariation(buttergotchiConfig.priColor), buttergotchiConfig.bgColor);
...
static void drawDualCoreInfo() {
    // Placed on the left after the time/version string so it doesn't clash with battery
    UBaseType_t taskCount = uxTaskGetNumberOfTasks();
    if (taskCount == 0 || taskCount > 64) {
        setTftDisplay(100, 12, buttergotchiConfig.priColor, 1, buttergotchiConfig.bgColor);
        tft.fillRect(100, 12, 50, LH, buttergotchiConfig.bgColor);
        tft.print("C0:C1");
        return;
    }
    ...
    char buf[20];
#if (configCORECOUNT > 1)
    snprintf(buf, sizeof(buf), "C0:%d C1:%d", c0, c1);
#else
    snprintf(buf, sizeof(buf), "C0:%d", c0);
#endif
    // Display after time/version text on the left side
    int leftEnd = 80; // approximate end of time/version text
    tft.fillRect(leftEnd, 12, 90, LH, buttergotchiConfig.bgColor);
    setTftDisplay(leftEnd, 12, buttergotchiConfig.priColor, 1, buttergotchiConfig.bgColor);
    tft.print(buf);
}
...
void drawStatusBar() {
    ...
    if (buttergotchiConfig.theme.border) {
        tft.drawRoundRect(5, 5, tftWidth - 10, tftHeight - 10, 5, buttergotchiConfig.priColor);
        tft.drawLine(5, 25, tftWidth - 6, 25, buttergotchiConfig.priColor);
    }

    if (clock_set) {
        setTftDisplay(12, 12, buttergotchiConfig.priColor, 1, buttergotchiConfig.bgColor);
        tft.fillRect(12, 12, 60, LH, buttergotchiConfig.bgColor);
#if defined(HAS_RTC)
        ...
        setTftDisplay(12, 12, buttergotchiConfig.priColor, 1, buttergotchiConfig.bgColor);
        String verStr = "HEAVYBUTTER " + String(BUTTERGOTCHI_VERSION);
        tft.print(verStr);
    }
    ...
    if (iconCount > 0) {
        ...
        // Reserve right-side space for battery icon (~42px wide)
        int coreInfoReserve = 48;
        ...
        if (showSD) {
            int x = sx + idx * (IW + GAP);
            tft.fillRect(x, iy, IW, IH, buttergotchiConfig.bgColor);
            drawSdSmall(x, iy);
            idx++;
        }
        if (showGPS) {
...
void printCenterFootnote(String text) {
    tft.fillRect(10, tftHeight - BORDER_PAD_X - FP * LH, tftWidth - 20, FP * LH, buttergotchiConfig.bgColor);
    tft.setTextSize(FP);
...
void drawBatteryStatus(uint8_t bat) {
    ...
    uint16_t color = buttergotchiConfig.priColor;
    uint16_t barcolor = buttergotchiConfig.priColor;
    if (bat < 16) barcolor = color = TFT_RED;

    tft.drawRoundRect(tftWidth - 42, 7, 34, 17, 2, color);
    tft.setTextSize(FP);
    tft.fillRect(tftWidth - 85, 7, 42, 18, buttergotchiConfig.bgColor);
    if (charging) {
        tft.setTextColor(buttergotchiConfig.priColor, buttergotchiConfig.bgColor);
    ...
    tft.drawLine(tftWidth - 30, 9, tftWidth - 30, 9 + 13, buttergotchiConfig.bgColor);
    tft.drawLine(tftWidth - 20, 9, tftWidth - 20, 9 + 13, buttergotchiConfig.bgColor);
}

void drawWireguardStatus(int x, int y) {
    tft.fillRect(x, y, 16, 16, buttergotchiConfig.bgColor);
    if (isConnectedWireguard) {
        tft.drawRoundRect(x + 4, y + 2, 8, 8, 3, buttergotchiConfig.priColor);
        tft.fillRoundRect(x + 3, y + 7, 10, 7, 1, buttergotchiConfig.priColor);
    } else {
        tft.drawRoundRect(x + 4, y + 2, 8, 8, 3, buttergotchiConfig.priColor);
        tft.drawRoundRect(x + 3, y + 7, 10, 7, 1, buttergotchiConfig.priColor);
    }
}
    ...
    while (i < arraySize) {
        if (i >= start) {
            ...
            if (fileList[i].folder == true)
                tft.setTextColor(getColorVariation(buttergotchiConfig.priColor), buttergotchiConfig.bgColor);
            else if (fileList[i].operation == true) tft.setTextColor(ALCOLOR, buttergotchiConfig.bgColor);
            else { tft.setTextColor(buttergotchiConfig.priColor, buttergotchiConfig.bgColor); }
            ...
            if (index == i) {
                ...
                coord.size = nchars;
                coord.fgcolor =
                    fileList[i].folder ? getColorVariation(buttergotchiConfig.priColor) : buttergotchiConfig.priColor;
                coord.bgcolor = buttergotchiConfig.bgColor;
            } else txt = " ";
...
void drawSdSmall(int x, int y) {
    tft.fillRect(x, y, 16, 16, buttergotchiConfig.bgColor);
    tft.drawLine(x + 3, y + 2, x + 3, y + 14, buttergotchiConfig.priColor);
    tft.drawLine(x + 3, y + 14, x + 13, y + 14, buttergotchiConfig.priColor);
    tft.drawLine(x + 13, y + 14, x + 13, y + 5, buttergotchiConfig.priColor);
    tft.drawLine(x + 13, y + 5, x + 10, y + 2, buttergotchiConfig.priColor);
    tft.drawLine(x + 10, y + 2, x + 3, y + 2, buttergotchiConfig.priColor);

    tft.drawLine(x + 5, y + 4, x + 5, y + 6, buttergotchiConfig.priColor);
    tft.drawLine(x + 7, y + 4, x + 7, y + 6, buttergotchiConfig.priColor);
    tft.drawLine(x + 9, y + 4, x + 9, y + 6, buttergotchiConfig.priColor);
    tft.drawLine(x + 11, y + 5, x + 11, y + 6, buttergotchiConfig.priColor);
}

void drawWifiSmall(int x, int y) {
    tft.fillRect(x, y, 16, 16, buttergotchiConfig.bgColor);
    tft.fillCircle(x + 8, y + 13, 1, buttergotchiConfig.priColor);
    tft.drawArc(x + 8, y + 13, 4, 6, 135, 225, buttergotchiConfig.priColor, buttergotchiConfig.bgColor);
    tft.drawArc(x + 8, y + 13, 9, 11, 135, 225, buttergotchiConfig.priColor, buttergotchiConfig.bgColor);
}

void drawWebUISmall(int x, int y) {
    tft.fillRect(x, y, 16, 16, buttergotchiConfig.bgColor);
    tft.drawCircle(x + 8, y + 8, 6, buttergotchiConfig.priColor);
    tft.drawLine(x + 3, y + 4, x + 13, y + 4, buttergotchiConfig.priColor);
    tft.drawLine(x + 2, y + 8, x + 14, y + 8, buttergotchiConfig.priColor);
    tft.drawLine(x + 3, y + 12, x + 13, y + 12, buttergotchiConfig.priColor);
}

void drawBLESmall(int x, int y) {
    tft.fillRect(x, y, 16, 16, buttergotchiConfig.bgColor);
    tft.drawWideLine(x + 8, y + 8, x + 4, y + 4, 2, buttergotchiConfig.priColor, buttergotchiConfig.bgColor);
    tft.drawWideLine(x + 8, y + 8, x + 4, y + 12, 2, buttergotchiConfig.priColor, buttergotchiConfig.bgColor);
    tft.drawTriangle(x + 8, y + 8, x + 8, y + 2, x + 12, y + 5, buttergotchiConfig.priColor);
    tft.drawTriangle(x + 8, y + 8, x + 8, y + 14, x + 12, y + 11, buttergotchiConfig.priColor);
}

void drawBLE_beacon(int x, int y, uint16_t color) {
    tft.fillRect(x, y, 40, 80, buttergotchiConfig.bgColor);
    tft.drawWideLine(40 + x, 53 + y, 2 + x, 26 + y, 5, color, buttergotchiConfig.bgColor);
    tft.drawWideLine(40 + x, 26 + y, 2 + x, 53 + y, 5, color, buttergotchiConfig.bgColor);
    tft.drawWideLine(40 + x, 53 + y, 20 + x, 68 + y, 5, color, buttergotchiConfig.bgColor);
    tft.drawWideLine(40 + x, 26 + y, 20 + x, 12 + y, 5, color, buttergotchiConfig.bgColor);
    tft.drawWideLine(20 + x, 12 + y, 20 + x, 68 + y, 5, color, buttergotchiConfig.bgColor);
...
void drawGPS(int x, int y) {
    tft.fillRect(x, y, 80, 80, buttergotchiConfig.bgColor);
    tft.drawEllipse(40 + x, 70 + y, 15, 8, buttergotchiConfig.priColor);
    tft.drawArc(40 + x, 25 + y, 23, 7, 0, 340, buttergotchiConfig.priColor, buttergotchiConfig.bgColor);
    tft.fillTriangle(40 + x, 70 + y, 20 + x, 64 + y, 60 + x, 64 + y, buttergotchiConfig.priColor);
}

void drawGpsSmall(int x, int y) {
    tft.fillRect(x, y, 16, 16, buttergotchiConfig.bgColor);
    tft.drawEllipse(x + 8, y + 13, 4, 3, buttergotchiConfig.priColor);
    tft.drawArc(x + 8, y + 5, 5, 2, 0, 360, buttergotchiConfig.priColor, buttergotchiConfig.bgColor);
    tft.fillTriangle(x + 8, y + 14, x + 4, y + 8, x + 12, y + 8, buttergotchiConfig.priColor);
}

void drawCreditCard(int x, int y) {
    tft.fillRect(x, y, 70, 50, buttergotchiConfig.bgColor);
    tft.fillRoundRect(x + 5, y + 5, 60, 40, 5, buttergotchiConfig.priColor);
    tft.fillRect(x + 5, y + 15, 60, 10, getColorVariation(buttergotchiConfig.priColor, 3, -1));
    tft.fillRect(x + 10, y + 30, 12, 10, getColorVariation(buttergotchiConfig.priColor, 3, 1));
    tft.drawRect(x + 10, y + 30, 12, 10, getColorVariation(buttergotchiConfig.priColor, 5, -1));
    tft.drawRect(x + 10 + 4, y + 30, 4, 10, getColorVariation(buttergotchiConfig.priColor, 5, -1));
    tft.drawRect(x + 10, y + 33, 5, 4, getColorVariation(buttergotchiConfig.priColor, 5, -1));
    tft.drawRect(x + 17, y + 33, 5, 4, getColorVariation(buttergotchiConfig.priColor, 5, -1));
    tft.fillRect(x + 30, y + 35, 30, 5, getColorVariation(buttergotchiConfig.priColor, 5, 1));
}

void drawMfkey32Icon(int x, int y) {
    tft.drawRect(x + 2, y + 15, 24, 40, buttergotchiConfig.priColor);
    tft.drawRect(x + 5, y + 18, 18, 12, buttergotchiConfig.priColor);
    tft.drawRect(x + 5, y + 34, 18, 18, buttergotchiConfig.priColor);
    tft.drawLine(x + 5, y + 40, x + 22, y + 40, buttergotchiConfig.priColor);
    tft.drawLine(x + 5, y + 46, x + 22, y + 46, buttergotchiConfig.priColor);
    tft.drawLine(x + 11, y + 34, x + 11, y + 51, buttergotchiConfig.priColor);
    tft.drawLine(x + 17, y + 34, x + 17, y + 51, buttergotchiConfig.priColor);
    tft.drawRect(x + 30, y + 10, 25, 35, buttergotchiConfig.priColor);
    int startX = x + 32;
    int startY = y + 12;
    int endX = x + 52;
    int endY = y + 32;
    ...
    while (startX <= endX && startY <= endY && turns < 3) {
        for (int i = startX; i <= endX; i++) { tft.drawPixel(i, startY, buttergotchiConfig.priColor); }
        startY += step;
        for (int i = startY; i <= endY; i++) { tft.drawPixel(endX, i, buttergotchiConfig.priColor); }
        endX -= step;
        for (int i = endX; i >= startX; i--) { tft.drawPixel(i, endY, buttergotchiConfig.priColor); }
        endY -= step;
        for (int i = endY; i >= startY; i--) { tft.drawPixel(startX, i, buttergotchiConfig.priColor); }
        startX += step;
        turns++;
    }
    tft.fillRect(x + 40, y + 36, 6, 6, getColorVariation(buttergotchiConfig.priColor, 3, 1));
}

void drawMfkey64Icon(int x, int y) {
    drawMfkey32Icon(x, y);
    tft.fillRoundRect(x + 40, y + 6, 24, 14, 4, buttergotchiConfig.bgColor);
    tft.drawRoundRect(x + 40, y + 6, 24, 14, 4, getColorVariation(buttergotchiConfig.priColor, 3, -1));
    tft.drawCircle(x + 48, y + 12, 4, getColorVariation(buttergotchiConfig.priColor, 3, -1));
}
...
int PNGDraw(PNGDRAW *pDraw) {
    ...
    uint8_t r = ((uint16_t)buttergotchiConfig.bgColor & 0xF800) >> 8;
    uint8_t g = ((uint16_t)buttergotchiConfig.bgColor & 0x07E0) >> 3;
    uint8_t b = ((uint16_t)buttergotchiConfig.bgColor & 0x001F) << 3;
...
bool drawPNG(FS &fs, String filename, int x, int y, bool center) {
    ...
    if (!mem) {
        ...
        buttergotchiConfig.theme.label = true;
        return false;
    }
