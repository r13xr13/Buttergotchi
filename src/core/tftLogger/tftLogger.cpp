...
tft_logger::tft_logger(int16_t w, int16_t h) : BUTTERGOTCHI_TFT_DRIVER(w, h) {}
...
void tft_logger::fillScreen(int32_t color) {
    ...
    BUTTERGOTCHI_TFT_DRIVER::fillScreen(color);
}
...
void tft_logger::drawLine(int32_t x, int32_t y, int32_t x1, int32_t y1, int32_t color) {
    ...
    if (isSleeping) return;
    BUTTERGOTCHI_TFT_DRIVER::drawLine(x, y, x1, y1, color);
    restoreLogger();
}
...
void tft_logger::drawRect(int32_t x, int32_t y, int32_t w, int32_t h, int32_t color) {
    ...
    if (isSleeping) return;
    BUTTERGOTCHI_TFT_DRIVER::drawRect(x, y, w, h, color);
    restoreLogger();
}
...
void tft_logger::fillRect(int32_t x, int32_t y, int32_t w, int32_t h, int32_t color) {
    ...
    if (isSleeping) return;
    BUTTERGOTCHI_TFT_DRIVER::fillRect(x, y, w, h, color);
    restoreLogger();
}
...
void tft_logger::drawRoundRect(int32_t x, int32_t y, int32_t w, int32_t h, int32_t r, int32_t color) {
    ...
    if (isSleeping) return;
    BUTTERGOTCHI_TFT_DRIVER::drawRoundRect(x, y, w, h, r, color);
    restoreLogger();
}
...
void tft_logger::fillRoundRect(int32_t x, int32_t y, int32_t w, int32_t h, int32_t r, int32_t color) {
    ...
    if (isSleeping) return;
    BUTTERGOTCHI_TFT_DRIVER::fillRoundRect(x, y, w, h, r, color);
    restoreLogger();
}
...
void tft_logger::drawCircle(int32_t x, int32_t y, int32_t r, int32_t color) {
    ...
    if (isSleeping) return;
    BUTTERGOTCHI_TFT_DRIVER::drawCircle(x, y, r, color);
    restoreLogger();
}
...
void tft_logger::fillCircle(int32_t x, int32_t y, int32_t r, int32_t color) {
    ...
    if (isSleeping) return;
    BUTTERGOTCHI_TFT_DRIVER::fillCircle(x, y, r, color);
    restoreLogger();
}
...
void tft_logger::drawEllipse(int16_t x, int16_t y, int32_t rx, int32_t ry, uint16_t color) {
    ...
    if (isSleeping) return;
    BUTTERGOTCHI_TFT_DRIVER::drawEllipse(x, y, rx, ry, color);
    restoreLogger();
}
...
void tft_logger::fillEllipse(int16_t x, int16_t y, int32_t rx, int32_t ry, uint16_t color) {
    ...
    if (isSleeping) return;
    BUTTERGOTCHI_TFT_DRIVER::fillEllipse(x, y, rx, ry, color);
    restoreLogger();
}
...
void tft_logger::drawTriangle(
    ...
    if (isSleeping) return;
    BUTTERGOTCHI_TFT_DRIVER::drawTriangle(x1, y1, x2, y2, x3, y3, color);
    restoreLogger();
}
...
void tft_logger::fillTriangle(
    ...
    if (isSleeping) return;
    BUTTERGOTCHI_TFT_DRIVER::fillTriangle(x1, y1, x2, y2, x3, y3, color);
    restoreLogger();
}
void tft_logger::drawArc(
    ...
    if (isSleeping) return;
    BUTTERGOTCHI_TFT_DRIVER::drawArc(x, y, r, ir, startAngle, endAngle, fg_color, bg_color, smoothArc);
    restoreLogger();
}
...
void tft_logger::drawWideLine(float ax, float ay, float bx, float by, float wd, int32_t fg, int32_t bg) {
    ...
    if (isSleeping) return;
    BUTTERGOTCHI_TFT_DRIVER::drawWideLine(ax, ay, bx, by, wd, fg, bg);
    restoreLogger();
}
...
void tft_logger::drawFastVLine(int32_t x, int32_t y, int32_t h, int32_t fg) {
    ...
    if (isSleeping) return;
    BUTTERGOTCHI_TFT_DRIVER::drawFastVLine(x, y, h, fg);
    restoreLogger();
}
...
void tft_logger::drawFastHLine(int32_t x, int32_t y, int32_t w, int32_t fg) {
    ...
    if (isSleeping) return;
    BUTTERGOTCHI_TFT_DRIVER::drawFastHLine(x, y, w, fg);
    restoreLogger();
}
...
int16_t tft_logger::drawString(const String &string, int32_t x, int32_t y, uint8_t font) {
    ...
    r = BUTTERGOTCHI_TFT_DRIVER::drawString(string, x, y, font);
    restoreLogger();
    return r;
}
...
int16_t tft_logger::drawCentreString(const String &string, int32_t x, int32_t y, uint8_t font) {
    ...
    int16_t r;
    if (isSleeping) return string.length();
    r = BUTTERGOTCHI_TFT_DRIVER::drawCentreString(string, x, y, font);
    restoreLogger();
    return r;
}
...
int16_t tft_logger::drawRightString(const String &string, int32_t x, int32_t y, uint8_t font) {
    ...
    int16_t r;
    if (isSleeping) return string.length();
    r = BUTTERGOTCHI_TFT_DRIVER::drawRightString(string, x, y, font);
    restoreLogger();
    return r;
}
...
size_t tft_logger::print(const String &s) {
    ...
    while (remaining > 0) {
        ...
        else totalPrinted += BUTTERGOTCHI_TFT_DRIVER::print(chunk);
