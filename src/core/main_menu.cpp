...
void MainMenu::begin(void) {
    ...
    std::vector<String> l = buttergotchiConfig.disabledMenus;
            ...
            options.push_back(
                {// selected lambda
                 ...
                 [](void *menuItem, bool shouldRender) { // render lambda
                     ...
                     if (buttergotchiConfigPins.rotation & 0b01) scale = float((float)tftHeight / (float)135);
                     obj->draw(scale);
...
RESTART: // using gotos to avoid stackoverflow after many choices
    ...
    for (auto item : items) {
        ...
        options.push_back({label, [this, label]() { buttergotchiConfig.addDisabledMenu(label); }, enabled});
    }
    options.push_back({"Show All", [=]() { buttergotchiConfig.disabledMenus.clear(); }, true});
    addOptionToMainMenu();
    loopOptions(options);
