  ...
  fileList
    ...
    .forEach((line) => {
      ...
      if (type === "pa") {
        ...
        if (IS_DEV) downloadUrl = "/api" + downloadUrl;
...
async function fetchSystemInfo() {
  ...
  $(".fw-version").textContent = info.FW_VERSION;
...
async function reloadScreen() {
  ...
  try {
    let binResponse = await fetch((IS_DEV ? "/api" : "") + "/getscreen");
...
$(".act-save-wpakey").addEventListener("click", async (e) => {
  let ssid = $("#wpakey-ssid").value.trim();
  let key = $("#wpakey-key").value.trim();
  if (!ssid || !key) {
    alert("SSID and WPA key cannot be empty.");
    return;
  }
  Dialog.loading.show("Saving WPA Key...");
  await requestGet("/wpakey", {
    ssid: ssid,
    key: key,
  });
  Dialog.loading.hide();
  alert("WPA key saved for " + ssid + "!");
  $("#wpakey-ssid").value = "";
  $("#wpakey-key").value = "";
});
