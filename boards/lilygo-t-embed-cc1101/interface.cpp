...
void _setup_gpio() {
    ...
    buttergotchiConfigPins.irRx = 1;
    buttergotchiConfigPins.irTx = 2;
#else
    Wire.begin(GROVE_SDA, GROVE_SCL);
    ...
    if (Wire.endTransmission() == 0) {
        ...
        buttergotchiConfigPins.CC1101_bus.cs = GPIO_NUM_17;
        buttergotchiConfigPins.CC1101_bus.io0 = GPIO_NUM_18;
        buttergotchiConfigPins.rfModule = CC1101_SPI_MODULE;
...
void InputHandler(void) {
    ...
    } else if (posDifference != 0 && millis() - lastEncoderMoveMs > 50) {
        posDifference = 0;
    }
    ...
    // Drain ALL accumulated encoder steps in one shot per InputHandler() call.
    // The old per-step 20ms throttle collided with check()'s vTaskSuspend + 10ms
    // delay, creating uneven bursts and jitter during fast rotation.
    //
    // RotaryEncoder with TWO03 latch mode can produce ambiguous intermediate
    // transitions at high speed, briefly reversing direction in posDifference.
    // To filter this bounce, we apply a direction-lock: if we see movement
    // in one direction and then a reversal within 30ms, the reversal is
    // treated as mechanical bounce and the net movement is kept.
    if (posDifference != 0) {
        static int lastDirection = 0;
        static unsigned long lastDirChangeMs = 0;
        int dir = (posDifference > 0) ? 1 : -1;

        if (dir != lastDirection && millis() - lastDirChangeMs < 30) {
            // Direction reversal within 30ms — likely mechanical bounce at
            // high speed.  Merge both directions into net movement.
            posDifference += dir * (lastDirection * -1);
        } else {
            lastDirection = dir;
    ...
    if (sel == BTN_ACT && millis() - lastEncoderMoveMs > 200) {
        SelPress = true;
        tm = millis();
    }
    if (esc == BTN_ACT) {
...
void checkReboot() {
    ...
    auto clearBanner = [&]() { tft.fillRect(bannerX, 7, bannerWidth, 18, buttergotchiConfig.bgColor); };

    // Helper function to clear text line only
    auto clearTextLine = [&]() {
        tft.fillRect(bannerX, bannerY, bannerWidth, bannerHeight, buttergotchiConfig.bgColor);
    };
        ...
        if (newCountDown != countDown || !bannerInitialized) {
            ...
            if (isRestartMode) {
                ...
                if (countDown >= RESTART_TIMEOUT + 1) {
                    ...
                    tft.setTextColor(buttergotchiConfig.secColor, buttergotchiConfig.bgColor);
