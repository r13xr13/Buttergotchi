    ...
    if (millis() - tm > 200 || LongPress) {
        ...
        if (touchInitialized) {
            ...
            if (ft6336_read_touch(raw_x, raw_y)) {
                ...
                } else if (buttergotchiConfigPins.rotation == 2) {
                    // Portrait inverted
                    t_x = (TFT_WIDTH - 1) - raw_x;
                    t_y = (TFT_HEIGHT - 1) - raw_y;
                } else if (buttergotchiConfigPins.rotation == 3) {
                    // Landscape inverted
...
void _setup_codec_mic(bool enable) {
    // Set microphone I2S pin for mic module
    ...
    if (enable) {
        ...
        // Clock config: MCLK derived from BCLK (bit7=1)
