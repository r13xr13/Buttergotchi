...
void _setup_gpio() {
    ...
    buttergotchiConfigPins.rotation = 0;  // portrait mode for Phantom
    buttergotchiConfig.colorInverted = 0; // color invert for Phantom
...
void _post_setup_gpio() {
    // uint16_t calData[5];
    // buttergotchiConfigPins.rotation = 0;
    // tft.setRotation(0);
...
void InputHandler(void) {
    ...
    if (millis() - tm > 200 || LongPress) {
        ...
        if (_IH_touched) {
            ...
            // buttergotchiConfigPins.rotation);
            tm = millis();
